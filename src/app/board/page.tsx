...
export default function BoardPage() {
  ...
  async function init() {
    const [profileRes] = await Promise.all([fetch("/api/user/profile")]);
  ...
  async function uploadPhoto(e: React.FormEvent) {
    ...
    if (res.ok) {
      setShowUpload(false);
      setUpTitle(""); setUpDesc(""); setUpFile(null); setUpPreview(null); setUpCat("기타");
      fetchPhotos();
    }
  }
                    ...
                    <div key={post.id} className="bg-gray-900 border border-white/5 rounded-2xl overflow-hidden">
                      ...
                      >
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <span className={`text-gray-500 text-xs shrink-0 transition-transform ${expanded ? "rotate-180" : ""}`}>▼</span>
                        </div>
                      </button>
                      {expanded && (
        ...
        <div className="fixed inset-0 bg-black/70 flex items-end sm:items-center justify-center z-50 px-4"
          ...
          <form onSubmit={uploadPhoto}
            ...
            <div className="px-5 py-4 space-y-3 max-h-[65vh] overflow-y-auto">
              ...
              {upPreview ? (
                <div className="relative rounded-xl overflow-hidden">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={upPreview} alt="" className="w-full h-44 object-cover" />
                  <button type="button"
                    onClick={() => { setUpFile(null); setUpPreview(null); if (fileRef.current) fileRef.current.value = ""; }}
                    className="absolute top-2 right-2 w-7 h-7 bg-black/60 text-white rounded-full text-sm flex items-center justify-center hover:bg-black/80">✕</button>
                </div>
              ) : (
                <button type="button" onClick={() => fileRef.current?.click()}
                  ...
                  <span className="text-xs font-medium">이미지 선택하기</span>
                </button>
              )}
              ...
              <div className="grid grid-cols-2 gap-2">
                <select value={upYear} onChange={e => setUpYear(Number(e.target.value))}
                  className={inputCls}>
            ...
            <div className="px-5 py-4 border-t border-white/5 shrink-0">
              <button type="submit" disabled={!upFile || !upTitle.trim() || uploading}
