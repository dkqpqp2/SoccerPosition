...
export async function POST(req: NextRequest) {
  ...
  const { match_date, match_time, match_end_time, location, title } = await req.json();

  const { data, error } = await supabaseAdmin
    .from("matches")
    .insert({ user_id: userId, team_id: teamId, match_date, match_time: match_time || null, match_end_time: match_end_time || null, location: location || null, title })
    .select()
    .single();
